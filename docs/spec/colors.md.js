...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = {
	  "content": ["article", ["p", "设计中对色彩的运用不仅应考虑品牌的识别性，还需达到信息传递、操作指引、交互反馈，或是强化和凸显某一个元素的目的。基于操作系统更注重高效、清晰等特点，Ant Design 的用色上更偏向简洁实用一些。在选择色彩时有以下三个注意点："], ["ul", ["li", ["p", "色彩应与产品定位相匹配，且符合用户心理认知；"]], ["li", ["p", "视觉层次应清晰分明，为重要行动点或关键信息定义一个主色，并建立视觉连续性；"]], ["li", ["p", "遵守 WCAG 2.0 的 标准，保证足够的对比度，让色彩更容易被视障碍（色盲）用户识别。"]]], ["h2", "Ant Design Colors"], ["p", "Ant Design PC 端的色板由 10 个由浅至深的色彩单元格组成，我们为部分色彩格定义了默认使用场景，用户在进行色彩配色时只需根据关键词选择一条色板即可得到一套完整的系统配色方案。在理论上，在 UI 设计中的色彩都应取自这份色板。"], ["p", "经过设计师和程序员的精心调教，结合了色彩加白、加黑、加深，贝塞尔曲线，以及针对冷暖色的不同旋转角度，得出一套", ["a", {
	    "title": null,
	  ...
	  }, "色板生成算法"], "。使用者只需指定主色，便可导出一条完整的渐变色板。"], ["p", "Ant Design 的色板由 8 种基本色彩组成，每种基本色（第 6 格）又按上述算法衍生出 10 种渐变色。"], ["blockquote", ["p", "注：在由浅至深的色板里，第 6 格色彩单元格普遍满足 ", ["a", {
	    "title": null,
	    "href": "http://leaverou.github.io/contrast-ratio/"
	  }, "WCAG 2.0"], " 的 4.5:1 最小对比度（AA 级），我们将其定义为色板的默认品牌色。"]], function jsonmlReactLoader() {
	    var rgbToHex = function rgbToHex(rgbString) {
	      var hexChars = '0123456789ABCDEF';var rgb = rgbString.match(/\d+/g);var r = parseInt(rgb[0]).toString(16);var g = parseInt(rgb[1]).toString(16);var b = parseInt(rgb[2]).toString(16);
	      r = r.length === 1 ? '0' + r : r;g = g.length === 1 ? '0' + g : g;b = b.length === 1 ? '0' + b : b;return '#' + r + g + b;
